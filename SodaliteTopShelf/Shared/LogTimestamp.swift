...
/// The UTC arrival stamp `LogTap` writes in front of every buffered line.
///
/// Computed straight from the epoch rather than through a `DateFormatter`, for two reasons. It sits on
/// the same per-line funnel as `LogRedaction`, where a Character-level pass once cost enough to shift
/// request timing in an AetherEngine test, so per-line work here is per-line for the whole session. And
/// a formatter carries a locale and a calendar: one that forgets `en_US_POSIX` prints Eastern Arabic
