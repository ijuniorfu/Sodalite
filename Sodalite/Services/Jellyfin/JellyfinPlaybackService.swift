...
protocol JellyfinPlaybackServiceProtocol: Sendable {
    ...
    /// URL for a Jellyfin-rendered chapter image (generated by the server's
    /// "Chapter image extraction" scheduled task). `chapterIndex` is the
    /// chapter's position in the item's original `Chapters` array. Returns
    /// nil if the server/token aren't known; the caller should fall back to
    /// decoding a still itself.
...
final class JellyfinPlaybackService: JellyfinPlaybackServiceProtocol {
    ...
    func buildChapterImageURL(itemID: String, chapterIndex: Int, imageTag: String, maxWidth: Int) -> URL? {
        ...
        components?.queryItems = [
            // `tag` selects this chapter's specific render and doubles as a
            // cache key, so a re-run of the scheduled task invalidates it.
