...
struct PlayerLauncher: UIViewControllerRepresentable {
    ...
    func updateUIViewController(_ host: PlayerLauncherHostVC, context: Context) {
        if isPresented, let item, host.presentedViewController == nil {
            // DIAGNOSTIC: when externalDiagnosticURL is set, skip the
            // AetherEngine pipeline entirely (no Demuxer / producer /
            // cache / loopback HTTP) and hand the URL straight to a
            // stock AVPlayerViewController. Use to A/B against the
            // engine path: if RSS stays flat on this route but climbs
            // on the engine route, the leak is in our pipeline. If
            // both routes climb, the leak is in AVPlayer's HLS-fMP4
            // demuxer regardless of who produced the segments.
            //
            // Set the LAN URL to the Mac http server hosting your
            // pre-produced HLS bundle (ffmpeg-cli with
            // -hls_playlist_type vod, served by python3 -m http.server
            // or similar). Nil for production.
...
/// HLSSegmentProducer / SegmentCache / HLSLocalServer) and hands the
/// URL directly to a stock AVPlayerViewController. Mirrors the
/// previous Part 2 isolation from AetherEngine#4 (commit cc6da97,
/// restored 2026-05-20 for the partial-recycle follow-up): if RSS
/// stays bounded on this route while the engine route still climbs,
/// the leak is in our producer / muxer / loopback HTTP stack. If
/// both climb at the same rate, the leak is in AVPlayer's own HLS
/// demuxer (or downstream of the URL the producer hands AVPlayer).
///
/// Note: AVKit's `appliesPerFrameHDRDisplayMetadata` defaults to
/// false on a plain item, and we don't toggle it here. That matches
/// the engine path on the media-playlist branch (HDR10 → media
/// playlist → perFrameHDR=false), so the comparison is apples-to-
/// apples for the suspected per-frame HDR retention path.
final class DiagnosticDirectURLPlayerVC: AVPlayerViewController {
    ...
    override func viewDidLoad() {
        ...
        // Leave AVPlayer's buffer policy at the defaults (matches the
        // production NativeAVPlayerHost defaults so the comparison
        // isolates the segment-source delta, not the buffer-policy
        // delta).
    ...
    /// 30 s memprobe loop, identical schema to the engine's so the
    /// numbers line up directly in side-by-side comparison. RSS via
    /// task_info phys_footprint (same as the engine), plus the same
    /// task_vm_info breakdown the engine logs.
    private func startMemprobe() {
        ...
        memprobeTask = Task { @MainActor [weak self] in
            while !Task.isCancelled {
                ...
                let physFP = Self.physFootprintMB()
                let vmInt = Self.vmInternalMB()
                let line = "[Diag] memprobe t=\(elapsed)s physFP=\(physFP)MB vmInt=\(vmInt)MB url=direct"
    ...
    private static func vmInternalMB() -> Int {
        var info = task_vm_info_data_t()
        var count = mach_msg_type_number_t(
            MemoryLayout<task_vm_info_data_t>.size / MemoryLayout<integer_t>.size
        )
        ...
        guard kerr == KERN_SUCCESS else { return 0 }
        return Int(info.internal / (1024 * 1024))
    }
}
