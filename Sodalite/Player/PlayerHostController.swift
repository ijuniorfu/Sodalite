...
final class PlayerHostController: AVPlayerViewController {
    ...
    #if os(iOS)
    /// Manual PiP trigger for the touch player's PiP button (auto-PiP is the AVPlayerViewController's own).
    /// Bound to the AVKit render layer when found; detached on the SW path. See PlayerPiPController.
    ...
    private func observeAVKitRenderLayer(for avPlayer: AVPlayer, attempt: Int = 0) {
        ...
        #if os(iOS)
        // Native backend has a render layer -> bind the manual-PiP controller + offer the PiP button.
