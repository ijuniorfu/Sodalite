...
- **Subtitles in Picture in Picture**: text subtitles render inside the PiP window (and on AirPlay / external displays) via native WebVTT renditions served by the engine, and they survive seeking in both directions. External subtitle files from your server join those renditions too, so they show in PiP like embedded tracks. Fullscreen keeps Sodalite's styled overlay; the native rendition engages only in PiP, so subtitles never double up.
