...
- **Click to pause**: an optional Apple TV setting for TV remotes that drive the box over HDMI-CEC, where Select is often the only key that gets through. With Settings → Playback → Click Toggles Playback on, a click pauses or resumes straight away instead of first waking the transport, which Up or Down still does. Off by default, so the Siri Remote flow stays as it is, and a remote's dedicated play/pause key has always toggled in a single press
- **Instant skip**: a companion Apple TV setting for the same CEC remotes. With Settings → Playback → Instant Skip on, left or right seeks right away instead of putting the target time on the bar and waiting for a click to confirm it, and letting go of a held key commits the spool the same way. Presses in quick succession still add up into a single seek, so tapping right three times moves once by three intervals rather than reloading three times. Off by default, the preview-then-confirm flow the Siri Remote is built around is untouched
