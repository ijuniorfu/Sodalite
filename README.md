...
- **Diagnostic Log**: Settings → Diagnostic Log shows the last 300 diagnostic lines from the current app session, so a bug report can carry the actual error instead of a description of it, with access tokens stripped from every line. Nothing is uploaded. On iPhone and iPad it can be copied; on Apple TV, Export shows a QR code that opens the log on a phone over the local network, served by the Apple TV itself for five minutes
