...
- **The plot on the first screen**: a detail page opens on three lines of the synopsis, not on a row of studio names, with year, runtime, ratings and format badges beside it and the actions below. An info button opens the full text together with every audio track and every subtitle the file carries, and a single line at the bottom of the page names the file, its size, codecs and bitrate
- **A play button that says what it will do**: it names the episode it would start, drops to the time left once there is something to resume, and reads Play Again on a title you have finished
- **Spoiler protection**: optionally blur episode images and episode, season or movie descriptions until you have started or watched a title, revealed with a press when you want to see one anyway. A series can override the setting in either direction from its detail page, always hidden or never hidden. Off by default, in Appearance settings
...
- **Full-bleed backdrops**: the first screen of a detail page is artwork, with the title, the plot and the actions over it. Scroll on and the artwork hands over to a page carrying its own colours, so cast, related titles and episode stills stay readable instead of competing with the fanart behind them. Titles without backdrop art get an ambient poster fill instead of a grey plate
...
- **Format badges on detail pages**: the metadata line of a movie or episode carries what the copy actually is, resolution (4K, 1080p), dynamic range (Dolby Vision, HDR10+, HDR10, HLG), audio codec (Dolby Digital+, TrueHD, DTS-HD, AAC) and spatial format (Atmos, DTS:X), read from the version you have selected rather than the first one on the server. HDR10+ is the one fact a library scan cannot deliver, because it rides inside the video packets rather than the container header, so a detail page showing an HDR10 copy asks the player engine directly, once per version, and raises the pill if the file carries it. On by default, and switchable in Appearance settings
...
### 🎬 Watch
- **Direct Play** for almost every codec on your server: H.264, HEVC, HEVC Main10, AV1, VP9, VP8, MPEG-4 Part 2 (XVID / DIVX), MPEG-2, VC-1, and the legacy Microsoft tail that old rips carry (MS-MPEG4 v1 / v2 / v3, the DivX 3.x era, plus WMV1 / WMV2 / WMV3), and the Flash tail (Sorenson Spark, On2 VP6). Containers: MKV, MP4, MOV, AVI, MPEG-TS, M2TS, VOB, 3GP, WebM, OGG, ASF / WMV including its WMA audio, and FLV including its Nellymoser, ADPCM and Speex audio. Server-side transcoding stays reserved for fringe codecs such as Theora and RealVideo.
...
- **Next episode**: auto-play with a countdown of 5 to 30 seconds, or none at all; configurable. The countdown sits either on the credits, where the switch cuts whatever is left of them, or on the end of the episode, where the card waits without a ring and nothing is cut. Turn the countdown off and the card stays without a timer either way, so credits and post-credit scenes play out before the next episode starts. Dismissing the card only clears it off the picture, the switch still happens at the end of the episode
...
- **Skip forward and back, separately**: each direction carries its own interval in Settings → Playback, 5, 10, 15 or 30 seconds, so reaching back for a line of dialogue can be a short hop while a jump ahead covers real ground. The Apple TV remote, the double tap on iPhone and iPad and an iPad keyboard's arrow keys all use them, and the on-screen readout names the length it actually moved
...
- **Hardware keyboard**: with a keyboard attached to an iPad, Space plays and pauses, a tap on the left or right arrow jumps the interval you configured for that direction, and holding one spools through the timeline with the same preview the remote gets, committing where you let go
...
- **Timeshift**: pause live TV and scrub back through the buffer with on-device frame previews, or snap back with Return to Live. How much a channel keeps is a Playback setting, ten minutes, thirty minutes, ninety minutes (the default) or three hours. Storage sets the real ceiling: a session keeps up to 2 GB and never more than a quarter of the free space, so a high-bitrate channel reaches back less far than the setting names. A pause that outlasts the buffer says what it cost rather than resuming somewhere else in silence
...
- **The tuner is given back**: a live session releases its tuner when the Apple TV goes to sleep or the television is switched off, not only when you leave the player by hand, and anything an earlier run stranded is closed at the next launch. Jellyfin never reclaims a live stream nobody closed, and an orphaned one counts against the tuner limit until the server restarts. A channel somebody else is watching is left alone
...
- **Requests** for movies, whole series or single seasons: one Request button opens a sheet with the season selection and the request options, and one confirm sends it
...
- **Multiple profiles**: keep more than one login remembered per server and switch between them without re-entering a password; the launch picker opens on your active profile first, and an optional setting brings the picker back automatically after the app has spent a while in the background (Settings → Profile). Long-press a profile there and pick Sign out on all devices to revoke its sign-in on the server as well, so it is signed out on every Apple TV, iPhone and iPad that shares it through iCloud; Log Out never revokes anything, since the synced sign-in is shared by all of them
- **Settings per profile**: every profile keeps its own playback and appearance settings (languages, subtitle style, skipping, spoiler protection, accent, background, badges), its own tabs and navigation style, and its own Home rows and library sorts, and they follow that profile to every device on the same iCloud account. What describes the device rather than the person watching (network and live buffers, the audio bridge, the Dolby Vision override, Stats for Nerds, the Top Shelf row) stays with the device, and the settings screens say which is which. A profile that is new on this device starts from the settings of the last profile used here
- **Parental controls**: set a Guardian PIN, then choose per profile whether it opens freely, needs a PIN to open, or needs the PIN to leave. A profile that needs a PIN to open can carry its own, so the PIN you type in front of the whole family does not also open the adult profiles; the Guardian PIN still opens everything and stays the only key to logging out, server management, tabs, Seerr, iCloud and the parental settings themselves. A locked device returns to the profile picker on a cold start, gated actions ask for the PIN first, the parental settings always do, and recovery goes through the Jellyfin password of a PIN-protected profile
- **Navigation style (Apple TV)**: Settings → Tabs also chooses whether tvOS navigates from the bar across the top or from a sidebar. The sidebar is Sodalite's own rather than the system's, which is what lets it carry your accent colour: tvOS renders its built-in sidebar monochrome and offers no way to tint it. It sits as an icon rail, expands to labels when you move into it, shows who is signed in at the top, and steps aside on sub-screens. Every screen gains the height the top bar reserves for itself, which the Live TV guide spends on another row. The choice applies when you leave the screen and syncs to a second Apple TV
...
- **Self-hosted certificates**: a server behind a self-signed or private-CA certificate works once you accept it. The connection attempt shows the certificate's SHA-256 fingerprint, you accept that one certificate for that one address, and it is remembered for that address alone, so trusting a LAN server does not lower the bar for the same server's public address. A certificate that later changes asks again rather than being accepted quietly, and there is no "allow anything" switch anywhere
- **Diagnostic Log**: Settings → Diagnostic Log shows the last 300 diagnostic lines from the current app session, so a bug report can carry the actual error instead of a description of it, with access tokens stripped from every line. It opens with the app, engine and system versions, and that line leads the copied and exported text as well, because a log is read against the build it came out of. Nothing is uploaded. On iPhone and iPad it can be copied; on Apple TV, Export shows a QR code that opens the log on a phone over the local network, served by the Apple TV itself for five minutes. For bugs that span a restart or a long session, the persistent log file switch (off by default) also writes every line to the device, capped at 32 MB and rotated so the oldest lines go first; the Apple TV export page then offers that whole file as a download, and iPhone and iPad share it through the share sheet
